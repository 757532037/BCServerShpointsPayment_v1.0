...
public class IsoField implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2457251764796710941L;
	...
	public void setByteValue(byte[] bts) throws UnsupportedEncodingException {//这里设置bytevalue值
		this.byteValue = bts;
		this.checked = true;
	...
	public void setValue(String value) throws UnsupportedEncodingException {//这里设置stringvalue值
		// 应用数据域被选中
		this.checked = true;
		this.value = value;
		// 格式化
		...
		if (this.value != null) {
			...
			case LLVAR:
			case LLLVAR:
				this.byteValue = this.value.getBytes(SimpleConstants.ENCODING);//GBK编码
				break;
			default:
				this.checked = false;
				break;
